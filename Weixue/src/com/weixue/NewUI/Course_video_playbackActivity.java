...
import com.weixue.weixueUI.VideoFullScreenActivity;
...
public class Course_video_playbackActivity extends Activity implements OnRatingBarChangeListener{
	...
	private boolean flag=false;//用于标记是否有视频数据
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(cid!=0){
			...
			getAllUnits(courseModel.getCourseID(), false);
		}
	}
	...
	private void init() {
		...
		courseModel=(CourseListModel) this.getIntent().getExtras().getSerializable("course");
		
		//getAllUnits(courseModel.getCourseID(), true);
		
		introduction=courseModel.getIntroduction();
		tv_introduction.setText(Html.fromHtml(introduction));
		courseImgUrl=courseModel.getCourse_ImgUrl();
		cid=courseModel.getCourseID();
		howMuch=courseModel.getCourse_LearnMoney();
		...
		float floatNum=Float.valueOf(courseModel.getPraiseDegree())/2;
	...
	OnClickListener OnClickListener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			case R.id.play:
				...
				if(flag){
					handler.sendMessage(handler.obtainMessage(SHOW_MESSAGE, "没有资源"));
				}
			...
			case R.id.img_videoImg:
				...
				intent.putExtra("content", courseModel.getDetailIntro());
				startActivity(intent);
				break;
			}
		}
