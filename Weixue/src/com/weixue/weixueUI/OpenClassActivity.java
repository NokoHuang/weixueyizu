...
public class OpenClassActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.open_class);
		init();
	...
	public void init(){
		mContext=this;
		lv_openClass=(ListView) findViewById(R.id.lesson_list);
		lv_openClass.setOnItemClickListener(onitemclicklistener);
		lv_openClass.setOnScrollListener(new MyScrollListener());
...
public void onScrollStateChanged(AbsListView view, int scrollState) {
    ...
    {
    	if((lv_openClass.getLastVisiblePosition()+1)==lv_openClass.getCount()&&lv_openClass.getCount()>=count&&isNotDownLoad){
    		isNotDownLoad=false;
    		//System.out.println("正在加载更多数据...");
    		lv_openClass.addFooterView(footerView_loading);
    		position=lv_openClass.getCount();
    		//System.out.println("position-->"+position);	    		
    		lv_openClass.setSelection(position);
    		pageIndex+=1;
		...
		public void handleMessage(Message msg) {
			...
			case SETDATA:{
				...
				try {
					...
					lv_openClass.addFooterView(footerView_loading);
					loadData(li_course);
					lv_openClass.removeFooterView(footerView_loading);
					break;
					case 0:
